...
const ViewSellingPolicies = () => {
  ...
  const columns = useMemo<ColumnDef<Cards_Props_TP>[]>(
    () => [
      ...
      },
      {
        ...
        cell: (info) =>
          info.row.original.min_discount_rate
            ? `${info.row.original.min_discount_rate} %`
            : info.row.original.min_discount_cash,
      },
      {
  ...
  const {
    ...
    endpoint: `/selling/api/v1/minimum_selling?page=${page}`,
    queryKey: ["allMinimum_Selling", page],
    pagination: true,
    onSuccess(data) {
        ...
        !dataSource.length && (
          <div className="flex justify-center items-center mt-32">
            <p className="text-lg font-bold">
              {t("there are no selling policies available yet")}
            </p>
          </div>
        )
      )}
