...
const SellingInvoiceDataInEdara = ({
  setStage,
  sellingItemsData,
  paymentData,
  clientData,
  ...
  console.log("🚀 ~ SellingInvoiceDataInEdara ~ paymentData:", paymentData)
      ...
      },
      {
        ...
        cell: (info) => {
          ...
          const finalKaratNamesOfSelsal = info.row.original.selsal
            ?.map((karat: any) => karat.karat_name)
            .join("-");
  ...
  const posSellingDataHandler = () => {
    ...
    const items = sellingItemsData?.map((item) => {
      ...
      const weightOfSelsal = item?.selsal?.reduce((acc: any, item: any) => {
        acc += +item.weight;
        return acc;
      }, 0);

      const costOfSelsal = item?.selsal?.reduce((acc: any, item: any) => {
        acc += +item.cost;
        return acc;
      }, 0);
    ...
    mutate({
      endpointName: "/sellingEdaraa/api/v1/add_Invoice",
...
export default SellingInvoiceDataInEdara;
