...
///
export const PayOffSecondScreen = ({
  setStage,
  selectedItem,
  setSanadId,
  ...
  if (!dataSource?.length)
    return (
      <div className="flex items-center justify-center py-16 flex-col">
        <h2 className="bg-mainGray text-mainGreen p-4 rounded font-bold text-xl mb-16">
          {t("there are no rejected items")}
        </h2>
        <Button bordered onClick={() => setStage(1)}>
          {t("back")}
        </Button>
      </div>
    );
  return (
    <>
        ...
        !!dataSource.length && (
          <div className="px-4 md:px-16 py-16">
            ...
            <div className="flex gap-x-4 mt-5">
              ...
              >
                {t("return all items to management")}
              </Button>
              <Button
                bordered
                onClick={() => {
      ...
      <Modal isOpen={openModal} onClose={() => setOpenModal(false)}>
        <div className="flex flex-col items-center justify-center gap-y-4">
          ...
          <div className="flex gap-x-4 mt-5">
            <Button
              onClick={() => {
                const allRows = selectedItem.items
                  .filter((item) => item.item_status === "Rejected")
                  .map((item) => item.hwya);
                ...
                mutate({
                  endpointName:
                    "branchManage/api/v1/restriction-items-rejected",
            ...
            <Button bordered onClick={() => setOpenModal(false)}>
              {t("cancel")}
            </Button>
          </div>
        </div>
      ...
      <Modal
        isOpen={retrieveOpenModal}
        onClose={() => setRetrieveOpenModal(false)}
      >
        <div className="flex flex-col items-center justify-center gap-y-4">
          <RiErrorWarningFill
            className="text-4xl scale-150 mb-5"
