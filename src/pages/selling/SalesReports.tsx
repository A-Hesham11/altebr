...
const SalesReports = () => {
  ...
  const { isLoading, isFetching, isRefetching, refetch } = useFetch({
    ...
    endpoint:
      search === `/report/api/v1/sales/${userData?.branch_id}?` || search === ""
        ? `/report/api/v1/sales/${userData?.branch_id}`
        : `${search}`,
    onSuccess: (data) => {
      setDataSource(data);
    },
  ...
  const getSearchResults = async (req: any) => {
    let url = `/report/api/v1/sales/${userData?.branch_id}?`;
    let first = false;
  ...
  if (isFetching && !search)
    return <Loading mainTitle={`${t("loading Sales Report")}`} />;
