...
export const PermissionForm = ({
  ...
  /////////// STATES
  ///
  const checkedStatus = !!!Object.entries(values).find(
    ([key, value]) => key !== "name" && (value === false || value === "")
  )?.length;
  ...
  return (
    <InnerFormLayout>
      <div className="flex items-center relative top-[104px]">
        <input
          type="checkbox"
          id="check_all"
          ...
          onChange={(e) => {
            Object.keys(values).map((value) => {
              if (value !== "name") setFieldValue(value, e.target.checked);
            });
          }}
          className="ml-2 text-mainGreen rounded"
        />
        <label htmlFor="check_all" className="text-md">
          {t("select all")}
        </label>
      </div>
      ...
      <CheckBoxField
        label={`${t("buying and selling policy")}`}
        name="is_selling"
        id="is_selling"
      />
