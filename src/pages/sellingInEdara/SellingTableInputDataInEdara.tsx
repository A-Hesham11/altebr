...
export const SellingTableInputDataInEdara = ({
  dataSource,
  setDataSource,
  sellingItemsData,
  setSellingItemsData,
  ...
  console.log("🚀 ~ SellingTableInputDataInEdara ~ dataSource:", dataSource)
  ...
  const isSearch = values.hwya.split("").map((item: any) => item)?.length >= 6;

  const { refetch, isSuccess, isFetching, isRefetching, isLoading } = useFetch({
    queryKey: ["edara-all-accepted-selling", values.hwya],
    endpoint: `/sellingEdaraa/api/v1/all-accepted?hwya[eq]=${values.hwya}`,
    onSuccess: (data) => {
      setDataSource(data);
    },
    onError: (error) => {
  ...
  const { data: karatValues } = useFetch<any>({
    endpoint: "classification/api/v1/allkarats",
    queryKey: ["karats_select"],
  });

  const sellingCols = useMemo<ColumnDef<any>[]>(
    () => [
      {
  ...
  useEffect(() => {
    if (dataSource?.length) {
      setKitDetails(
        dataSource?.find((item: Selling_Invoice_TP) => item.weightitems)
          ?.weightitems
      );
    }
  }, [dataSource]);
          ...
          <tr className="border-b text-center table-shadow last:shadow-0">
            ...
            <td className="border-l-2 border-l-flatWhite">
              <SelectKarat
                ...
                placement="top"
                onChange={(option) => {
                  const fieldKey = "karat_name" || "karatmineral_name";
                  setFieldValue(fieldKey, option!.value) as any;
                  setFieldValue(
                    "stock",
                    karatValues!.find(
                      (item: any) => item.karat === values.karat_value
                    )?.value
                  );
                }}
                value={{
              ...
              {values?.category_type === "multi" && (
                <Button
                  ...
                  action={() => {
                    ...
                    const pieceCheck = sellingItemsData?.findIndex(
                      (item: any) => {
