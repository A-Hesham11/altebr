...
///
export const HonestBondAccountingRestriction = ({
  sanadId,
}: {
  sanadId?: number;
}) => {
  ...
  const {
    ...
    endpoint: `branchSafety/api/v1/bond-boxes/${sanadId ? sanadId : bondId}`,
    queryKey: [`honest-bond-restrict-${sanadId ? sanadId : bondId}`],
    pagination: true,
    select: (contract: any) => {
      return {
    ...
    onError: (error) => {
      error && notify("info", error?.response?.data.errors.status);
    },
  });
  ...
  return (
    <div className="px-16 mt-8">
      ...
      {contractIsSuccess && !contractIsLoading && (
        <>
          <div className="p-8 rounded bg-white shadow-lg mb-4">
            <ul className="columns-3 list-disc">
              <li>
                ...
                {contract.userRestrictionData.client_id}
              </li>
              <li>
                <span className="font-bold">{t("employee name")}: </span>
                {contract.userRestrictionData.employee_id}
              </li>
              <li>
                <span className="font-bold">{t("items count")}: </span>
                {contract.userRestrictionData.count_items}
              </li>
              <li>
                <span className="font-bold">{t("date")}: </span>
                {new Date(contract.userRestrictionData.bond_date)
                  .toISOString()
                  .slice(0, 10)}
              </li>
            </ul>
          </div>
          <Table data={dataSource} columns={cols1}>
          ...
          {!contract?.boxes.length ? (
            <p className="font-bold text-center mt-16 text-xl">
              {t("there are no restriction to this bond")}
            </p>
          ) : (
            ""
          )}
