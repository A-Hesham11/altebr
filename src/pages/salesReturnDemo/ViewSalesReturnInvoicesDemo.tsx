...
import SellingInvoiceTablePreviewDemo from "../../components/selling/selling components/SellingDemo/ShowSellingDemo/SellingInvoiceTablePreviewDemo";

const ViewSalesReturnInvoicesDemo = () => {
  // STATE
  ...
  const {
    ...
    endpoint:
      search === `returnSale/api/v1/all-selling/${userData?.branch_id}` ||
      search === ""
        ? `returnSale/api/v1/all-selling/${userData?.branch_id}?page=${page}`
        : `${search}`,
    pagination: true,
  });
  ...
  const getSearchResults = async (req: any) => {
    let url = `returnSale/api/v1/all-selling/${userData?.branch_id}?`;
    let first = false;
  ...
  return (
    <div className="p-12">
      ...
      <Modal isOpen={invoiceModal} onClose={() => setOpenInvoiceModal(false)}>
        <SellingInvoiceTablePreviewDemo item={selectedItem} />
      </Modal>
    </div>
  );
};

export default ViewSalesReturnInvoicesDemo;
