...
import AddSellingPolicies from './AddExcludedItems'
...
const ViewExcludedItems = () => {
  ...
  useFetch<Cards_Props_TP[]>({
    endpoint:`/selling/api/v1/excluded_items`,
    queryKey: ["allExcludedItems"],
    pagination: true,
    onSuccess(data) {
  ...
  const handleDelete = () => {
    mutate({
      endpointName: `/selling/api/v1/delete_excluded_item/${deleteData?.id}`,
      method: "delete",
    })
  }
  ...
  return (
    <div className=''>
      <div className='flex justify-between items-center mb-8'>
        <p className='font-semibold text-lg'>{t("view excluded items")}</p>
      ...
      {isFetching && (<Loading mainTitle={t("excluded items")} />)}
...
export default ViewExcludedItems