...
function TreeItem<T = any>({
  ...
  const { style: propStyle, ...otherProps } = { ...props, ...itemProps };

  const mergedStyle = {
    ...propStyle,
    "--tree-padding": `${item.getItemMeta().level * indent}px`,
    paddingInlineStart: "var(--tree-padding)",
  } as React.CSSProperties;
...
function TreeDragLine({
  ...
  return (
    <div
      style={dragLine}
      className={cn(
        "bg-mainGreen before:bg-background before:border-mainGreen absolute z-30 -mt-px h-0.5 w-[unset] before:absolute before:-top-[3px] before:left-0 before:size-2 before:rounded-full before:border-2",
        className
      )}
      {...props}
    />
