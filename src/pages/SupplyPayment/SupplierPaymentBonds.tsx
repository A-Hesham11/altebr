...
const SupplierPaymentBonds = () => {
  ...
  const {
    data: supplierPaymentBonds,
    isLoading,
    isFetching,
    isRefetching,
    refetch,
  } = useFetch({
    queryKey: ["payment-supplier-boxes"],
    endpoint:
      search === ""
        ? `/sadadSupplier/api/v1/sdadbonds/${userData?.branch_id}?page=${page}`
        : `${search}`,
    pagination: true,
  });
  ...
  // EFFECTS
  useEffect(() => {
    if (supplierPaymentBonds) {
      setDataSource(supplierPaymentBonds.data);
    }
  }, [supplierPaymentBonds]);

  useEffect(() => {
    refetch();
  }, [page, supplierPaymentBonds]);
...
export default SupplierPaymentBonds;