...
const RecieveItemsFirstScreen = ({
  ...
  return (
    <div className="px-2 md:px-16 py-2 md:py-8">
          ...
          <div>
            <div className="flex justify-between">
              <div className="flex gap-4 mb-4">
                ...
                />
                <Button
                  className={`text-sm bg-mainOrange text-white ${
                    sortItems &&
                    "bg-white text-mainOrange border-mainOrange border"
                  }`}
                  type="button"
                  onClick={() => {
                    setSortItems(false);
      ...
      {!dataSource.length && !isRefetching && !isLoading && !sortRefetching && (
        <h2 className="font-bold text-xl mx-auto my-8 text-mainGreen bg-lightGreen p-2 rounded-lg w-fit">
          {t("no results")}
        </h2>
      )}
      {(isLoading || isRefetching || sortRefetching || sortLoading) && (
        <Loading mainTitle={t("loading items")} />
      )}
      {!data?.data.length && isRefetching && (
