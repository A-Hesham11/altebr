...
import { ItemDetailsStoneTable } from "../../../components/selling/recieve items/itemsDetailsStoneTable";
import { MissingItemsStoneDetailsTable } from "./MissingItemsStoneDetailsTable";

export const MissingItemsDetails = ({
  selectedItem,
  ...
  const selectedItemDetails = Array.isArray(selectedItem)
    ? selectedItem?.filter(
        (item: any) => item.item_id !== selectedRowDetailsId
      )[0]?.detailsItem
    : selectedItem?.items?.filter(
        (item: any) => selectedItem?.id !== selectedRowDetailsId
      )[0]?.detailsItem;

  const detailsWeightOfItem = selectedItem?.items?.filter(
    (item: any) => item?.id === selectedRowDetailsId
  )[0]?.weightitems;
    ...
    <div>
      ...
      {detailsWeightOfItem?.length ? (
        <div className="my-4">
          ...
          <Table data={detailsWeightOfItem} columns={Cols2}></Table>
        </div>
      ) : (
        ""
      )}
      <div className="mt-2 md:mt-8 grid grid-cols-12 items-center">
        <div className="col-span-12">
          <MissingItemsStoneDetailsTable
            selectedItem={selectedItem}
            selectedRowDetailsId={selectedRowDetailsId}
          />
        </div>
        <div className="col-span-1 ms-4">
          <FilesPreviewOutFormik images={images} pdfs={[]} preview />
        </div>
      </div>
        
    </div>
