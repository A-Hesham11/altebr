...
const ShowSellingInvoiceInEdara = () => {
  // STATE
  const isRTL = useIsRTL();
  const [dataSource, setDataSource] = useState([]);
  console.log("🚀 ~ ShowSellingInvoiceInEdara ~ dataSource:", dataSource);
  ...
  const {
    ...
    queryKey: ["edara-selling-invoices"],
    endpoint:
      search === `sellingEdaraa/api/v1/invoices` || search === ""
        ? `sellingEdaraa/api/v1/invoices?page=${page}`
        : `${search}`,
    pagination: true,
  });
  ...
  const getSearchResults = async (req: any) => {
    let url = `sellingEdaraa/api/v1/invoices?`;
    let first = false;
...
export default ShowSellingInvoiceInEdara;
