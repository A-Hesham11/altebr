...
const AddEntitlementsPolicies = ({ title, editData, setShow, refetch }) => {
  ...
  function PostNewCard(values) {
    mutate({
      endpointName: "employeeSalary/api/v1/entitlements",
      values,
      method: "post",
    });
  }
  ...
  const PostCardEdit = (values) => {
    mutate({
      endpointName: `employeeSalary/api/v1/entitlements/${editData?.id}`,
      values: {
        ...values,
        _method: "put",
      },
  ...
  return (
    <>
      <OuterFormLayout header={t("add entitlements policy")}>
        <Formik
              ...
              <div className="grid grid-cols-3 gap-x-6 gap-y-4 items-end mb-8">
                <div>
                  <BaseInputField
                    ...
                    name="name_ar"
                    type="text"
                    label={`${t("entitlement name in arabic")}`}
                    placeholder={`${t("entitlement name in arabic")}`}
                  />
                </div>
                <div className="relative">
                  <BaseInputField
                    ...
                    type="text"
                    name="name_en"
                    label={`${t("entitlement name in english")}`}
                    placeholder={`${t("entitlement name in english")}`}
                    className="relative"
                  />
                </div>
                <Select
...
export default AddEntitlementsPolicies;
