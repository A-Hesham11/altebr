...
const AddTaxPolicy = ({ editData }: SellingPoliciesProps_TP) => {
  const [dataSource, setDataSource] = useState<Cards_Props_TP[]>([]);
  // const [editTax, setEditTax] = useState([])
  // const [addTaxes, setAddTaxes] = useState([]);
  const [selectBranch, setSelectBranch] = useState("");

  const includeTaxFilter = dataSource?.filter(
    (item) => item.branch_id == selectBranch
  );
  ...
  const { isSuccess, refetch } = useFetch<Cards_Props_TP[]>({
  ...
  return (
    <>
      <OuterFormLayout header={t("Add Tax Policy")}>
        <Formik
                  ...
                  <div>
                    <BaseInputField
                      ...
                      onChange={(e) => {
                        ...
                        setFieldValue(
                          "include_tax",
                          +includeTaxFilter[0]?.include_tax
                        );
                      }}
                    />
                  </div>
                    ...
                    <RadioGroup name="include_tax">
                      <div className="flex gap-x-2 font-bold">
                        <RadioGroup.RadioButton
                          ...
                          isChecked={
                            values?.branch_id && includeTaxFilter.length !== 0
                              ? +includeTaxFilter[0]?.include_tax == 0
                                ? false
                                : true
                              : editData
                              ? editData?.include_tax == 0
                                ? false
                                : true
                              : ""
                          }
