...
export const GoldCodingSanadPicker = () => {
  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      Cell: (info) =>
        !isRTL ? info.value : info.value === "gold" ? "ذهب" : "gold",
    },
    {
      name: "supplier_name",
    ...
    },
    {
      ...
      label: t("item status"),
      Cell: (info) =>
        !isRTL ? info.value : info.value === "open" ? "مفتوح" : "مرقمة",
    },
    {
      name: "item_count",
      label: t("item count"),
  ...
  const {
    ...
    endpoint:
      search === "" ? `tarqimGold/api/v1/open-bonds?page=${page}` : `${search}`,
    ...
    select(data) {
      if (isOpenBond) {
        const openBonds = data?.data
          .filter((bond: GoldSanad_TP) => bond.trqem_status === "open")
          .map((item, i) => ({
            ...item,
            index: i + 1,
          }));
        ...
        const closeBonds = data?.data
          .filter((bond: GoldSanad_TP) => bond.trqem_status === "closed")
          .map((item, i) => ({
            ...item,
            index: i + 1,
          }));
  ...
  return (
    <>
      <div className="flex flex-col gap-4 rounded-lg bg-opacity-50 mb-5">
        ...
        <Header
          header={`${t("choose gold bond supply")}`}
      ...
      {!isFetching && !isLoading && !dataSource.length && (
        <h2 className="text-center font-bold text-xl bg-mainGreen text-white py-4 px-8 rounded-xl w-fit mx-auto">
          {t("there is no items")}
        </h2>
      )}
      {isSuccess &&
