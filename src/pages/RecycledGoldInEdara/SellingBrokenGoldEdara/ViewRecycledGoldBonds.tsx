...
import SellingInvoiceRecycledGoldPreview from "./SellingInvoiceRecycledGoldPreview";
...
const ViewRecycledGoldBonds = () => {
  ...
  const {
    ...
    queryKey: ["sellingInvoiceRecycledGold"],
    endpoint:
      search === `oldGold/api/v1/invoices` || search === ""
        ? `oldGold/api/v1/invoices?page=${page}`
        : `${search}`,
    pagination: true,
  });
  ...
  const tableColumn = useMemo<any>(
    () => [
      ...
      {
        cell: (info: any) =>
          info.getValue() ? info.getValue() : info.row.original.supplier_name,
        accessorKey: "client_name",
        header: () => (
          <span>
            {t("client name")} / {t("supplier")}
          </span>
        ),
      },
      {
  ...
  const tableColumnEntry = useMemo<any>(
    () => [
      {
        cell: (info: any) => info.getValue(),
      ...
      },
      {
        cell: (info: any) =>
          formatReyal(
            Number(info.renderValue()) - Number(info.row.original.vat)
          ) || "---",
        accessorKey: "total",
  ...
  return (
    <div className="">
      ...
      <Modal isOpen={invoiceModal} onClose={() => setOpenInvoiceModal(false)}>
        <SellingInvoiceRecycledGoldPreview item={selectedItem} />
      </Modal>
...
export default ViewRecycledGoldBonds;
