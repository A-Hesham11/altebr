...
const SupplierPaymentProccessingTable = ({
          ...
          <span className="flex items-center justify-center gap-3">
            ...
            <DeleteIcon
              ...
              action={() => {
                ...
                const stockDifference = info.row.original.stock_difference;
                setPaymentData((curr) => {
                  return curr.filter((item) => {
                    return item.stock_difference
                      ? item.id !== itemId ||
                          item.stock_difference != stockDifference
                      : item.id !== itemId;
                  });
                });
              }}
  ...
  return (
    <div>
      <p className="text-base font-bold">{`${t("Payment operations")} : ${
        paymentData?.length === 0 ? t("none") : paymentData?.length
      }`}</p>
            ...
            {table.getRowModel().rows.map((row) => (
              <tr key={row.id} className="text-center">
                {row.getVisibleCells().map((cell, i) => (
                  <td
                    className={` ${
                      editingRowId == cell.row.original.id
                        ? "bg-flatWhite"
                        : "bg-lightGray"
                    }
...
export default SupplierPaymentProccessingTable;
