    ...
    onSuccess(data: { id: number } | undefined) {
      ...
      navigate(
        supply === "gold"
          ? `/gold-bonds/${data!.id}`
          : supply === "diamond"
          ? `/diamond-bonds/${data!.id}`
          : `/accessory-bonds/${data!.id}`
      );
    },
  });
  ///
      ...
      ? (item: any) => {
          ...
          switch (item.front_key) {
            ...
            case "goldpurities":
              ...
              finalData!.table.forEach((row) => {
                total_24_gold_by_stock =
                  total_24_gold_by_stock +
                  Number(row.weight) * Number(row.stock);
              });
            ...
            case "gold3yar":
              ...
              let value =
                finalData!.boxes.total_weight - total_24_gold_by_karat2;
              if (value < 0) {
                value = value * -1;
                const computational_movement =
                  item.nature === "debtor" ? "creditor" : "debtor";
          ...
          switch (item.front_key) {
            ...
            case "supplier_money":
              return {
                ...item,
                value:
                  finalData!.boxes.total_tax +
                  finalData!.boxes.total_diamond_value,
              };
            default:
              return item;
          }
          ...
          switch (item.front_key) {
            ...
            case "supplier_money":
              return {
                ...
                value:
                  finalData!.boxes.total_tax +
                  finalData!.boxes.total_accessories_value,
              };
            default:
              return item;
          }
      ...
      supply === "gold"
        ...
        ? {
            ...
            total_other_stones_weight:
              finalData?.boxes.total_other_stones_weight / 5,
            total_diamond_number: finalData?.boxes.total_diamond_amount,
            total_diamond_stone_weight:
              finalData?.boxes.total_diamond_stone_weight / 5,
          }
        : {
            twred_type: formValues?.twred_type,
            bond_date: formatDate(formValues!.bond_date),
