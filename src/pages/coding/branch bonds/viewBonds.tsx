...
export const ViewBonds = () => {
  ...
  if (branchesWithoutEdara && !branchesLoading) {
    branchesWithoutEdara.forEach((branch) => {
      const countBranchesReyal = countBranchesOptions?.find(
        (item) => item.branch_id === branch.id && item.unit_id === "ريال"
      );
      const countBranchesGram = countBranchesOptions?.find(
        (item) => item.branch_id === branch.id && item.unit_id === "جرام"
      );
      ...
      systemCards.push({
        ...
        viewCountReyal: countBranchesReyal
          ? `${formatReyal(countBranchesReyal.value)} ${
              countBranchesReyal.unit_id
            }`
          : "",
        viewCountGram: countBranchesGram
          ? `${formatGram(countBranchesGram.value)} ${
              countBranchesGram.unit_id
            }`
          : "",
      });
    });
