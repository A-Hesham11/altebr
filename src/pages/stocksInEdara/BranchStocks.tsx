...
const BranchStocks = () => {
  ...
  const {
    ...
    endpoint: `/branchAccount/api/v1/getAllAccountBranches?page=${page}`,
    select: (data: object[]) => {
      return {
        ...data,
  ...
  useEffect(() => {
    if (branchCredit) {
      setDataSource(branchCredit?.data);
    }
  }, [branchCredit]);
  ...
  return (
    <div className="">
      <div className="flex justify-between items-center mb-8">
        <h2 className=" text-base font-bold">{t("branch credits")}</h2>
      </div>
            ...
            <div className="mt-3 flex items-center justify-center gap-5 p-2">
              <div className="flex items-center gap-2 font-bold">
                ...
                <span className=" text-mainGreen">
                  {branchCredit?.current_page}
                </span>
                {t("from")}
                {<span className=" text-mainGreen">{branchCredit?.pages}</span>}
              </div>
              <div className="flex items-center gap-2 ">
                ...
                <Button
                  className="rounded bg-mainGreen p-[.18rem]"
                  action={() => setPage((prev) => prev + 1)}
                  disabled={page == branchCredit?.pages}
                >
                  {isRTL ? (
                    <MdKeyboardArrowLeft className="h-4 w-4 fill-white" />
                  ) : (
